...
function progress(i, x){
    let hexAdds = [80+(i*(25+(i*5))), 199+(i*7), 255-(i*(15+(i*5)))]
    progressValues[i]=x;
...
function makeCircleEffectText(){
    ...
    for (let i = 2; i < data.numbers.length; i++) {
        string += `Circle ${i+1} (${formatWhole(data.numbers[i])}%): ${format(higherEffects[i-2])}x to the effect of Circle ${i}.\n`
    }
    return string
}
...
function circleTextControls(){
    ...
    document.getElementById("descText2").innerHTML = `All circles above Circle 2 multiply the effect of the previous Circle.<br><span style="font-size: 0.9rem">${data.jokes[data.getRandomInt(9)]}</span>`
